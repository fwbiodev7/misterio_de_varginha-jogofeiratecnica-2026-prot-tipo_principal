...
def hair_side(img,cx,cy,rx,ry,d,l):
    ...
    rc(img,cx-rx,cy-ry,rx+4,5,l)
    rc(img,cx-rx,cy-ry,2,ry+3,d)
    pt(img,cx-rx+2,cy-ry+5,INK)

def hair_bangs(img,cx,cy,rx,ry,d,l):
    disk(img,cx,cy+1,rx,ry,d)
    rc(img,cx-rx,cy-ry+1,rx*2+1,7,d)
    rc(img,cx-rx+2,cy-ry+1,rx*2-3,2,l)
    rc(img,cx-rx,cy-ry+1,rx*2+1,2,d)

def hair_buzz(img,cx,cy,rx,d,l):
    disk(img,cx,cy-1,rx,5,d)
    rc(img,cx-rx+2,cy-4,rx*2-3,2,l)

def hair_cpart(img,cx,cy,rx,ry,d,l):
    disk(img,cx,cy-1,rx,ry,d)
    rc(img,cx-rx+1,cy-ry,rx-2,6,l)
    rc(img,cx+2,cy-ry,rx-2,6,l)
    rc(img,cx-1,cy-ry-1,3,ry+3,d)
    rc(img,cx-rx,cy-1,3,9,d)
    rc(img,cx+rx-2,cy-1,3,9,d)
...
def face(img,cx,cy,sk,mc=(195,90,85,255)):
    ...
    # nose bridge
    pt(img,cx,cy+2,(int(sk[0]*0.85),int(sk[1]*0.85),int(sk[2]*0.85),255))
    # mouth
    rc(img,cx-2,cy+4,5,2,mc)
    pt(img,cx-1,cy+5,(int(mc[0]*0.75),int(mc[1]*0.75),int(mc[2]*0.75),255))
    pt(img,cx+1,cy+5,(int(mc[0]*0.75),int(mc[1]*0.75),int(mc[2]*0.75),255))
...
def gls(img,cx,cy):
    ...
    for dx in(-5,-4,-3,2,3,4):
        p=img.load();p[cx+dx,cy-1]=G_LENS
        p[cx+dx,cy]=G_LENS
...
def body(img,cx,cy,w,h,sl,sd):
    ...
    # collar highlight
    rc(img,cx-w//2+1,cy,w-2,2,(min(255,sl[0]+40),min(255,sl[1]+40),min(255,sl[2]+40),255))

def beard(img,cx,cy):
    b=(20,14,10,255)
    rc(img,cx-4,cy+2,9,3,b)
    rc(img,cx-7,cy,2,7,b)
    rc(img,cx+5,cy,2,7,b)
    rc(img,cx-6,cy+5,13,5,b)
    # mustache gap (lips)
    pt(img,cx-1,cy+3,(int(SKIN['messias'][0]*0.8),int(SKIN['messias'][1]*0.8),int(SKIN['messias'][2]*0.8),255))
    pt(img,cx+1,cy+3,(int(SKIN['messias'][0]*0.8),int(SKIN['messias'][1]*0.8),int(SKIN['messias'][2]*0.8),255))
...
def hband(img,cx,cy,cm,cl):
    ...
    pt(img,cx-10,cy-1,cl);pt(img,cx-10,cy+1,cl)

def earring(img,x,y):
    pt(img,x,y,(252,215,55,255))
    pt(img,x,y+1,(232,190,35,255))
    pt(img,x+1,y+1,(252,215,55,255))

def seat_bg(img):
    for y in range(58,205):
        shade = max(0, min(255, int(SEAT_MID[0] - (y-58)*0.25)))
        shade_g = max(0, min(255, int(SEAT_MID[1] - (y-58)*0.18)))
        shade_b = max(0, min(255, int(SEAT_MID[2] - (y-58)*0.12)))
        rc(img, 120, y, 263, 1, (shade, shade_g, shade_b, 255))
    # Back rest top edge
    rc(img,120,58,263,3,SEAT_DARK)
    # Seat divider (front/back rows)
    rc(img,120,145,263,4,SEAT_DARK)
    rc(img,120,146,263,2,SEAT_LIGHT)

MC=dict(tavares=(195,90,80,255),sabia=(195,88,88,255),marcos=(150,68,55,255),
...
def draw(img,cx,cy,nm,sty,wide=False):
    ...
    elif sty=='curly_tall':hair_curly(img,cx,cy-12,14,16,d,l)
    ...
    if nm=='marcos':
        fd=(88,58,35,255)
        rc(img,cx-10,cy-3,3,8,fd);rc(img,cx+8,cy-3,3,8,fd)
...
def compose():
    ...
    seat_bg(img)
    # Back row  (y=88): Tavares | Fabio | Sabia | Marcos | Martins
    draw(img,140,86,'tavares','curly')
    draw(img,178,86,'fabio','cpart')
    draw(img,218,86,'sabia','curly',wide=True)
    draw(img,258,86,'marcos','afro')
    draw(img,298,86,'martins','side')
    # Front row (y=118): Yasmin | Pedro | Matias | Messias
    draw(img,148,116,'yasmin','bangs')
    draw(img,194,116,'pedro','curly_tall')
    draw(img,238,116,'matias','curly_hband')
    draw(img,282,116,'messias','buzz')
    # Window pillar shadows
    rc(img,118,58,3,100,(30,48,62,255))
    rc(img,382,58,2,100,(30,48,62,255))
